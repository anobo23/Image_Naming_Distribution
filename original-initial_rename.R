...
rn.1st <- function(place){
                ...
                i.path <- paste0("~/Dropbox (My working)/team_folder/raw_image/", place)
                ...
                file.list <- file.list %>% 
                        mutate(NewName = paste0("-", format(file.time, format = "%y%m%d"), "-", place, "-", FileName),
                               NewDirectory = paste0("~/Dropbox (My working)//team_folder/raw_image/temp_for_rename/", NewName))
